var Audio = {
  ...
  selectPerformer: function(opts) {
    opts = opts || {};
    var __cur = opts.from_pad ? window._pads && _pads.cur : window.cur,
        from_pad = !!opts.from_pad, event = opts.event, name = opts.name;
  ...
  changeHTitle: function(opts) {
    ...
    if (__cur.curSection && !__cur.curSection.indexOf('album')) {
      ...
      if (album) {
        htitle = album.innerText || album.textContent;
      }
    } else if (__cur.curSection && __cur.curSection == 'recommendations') {
      htitle = __cur.recommendTitle ? __cur.recommendTitle : getLang('audio_recommended_audios');
    } else if (__cur.curSection && __cur.curSection == 'feed') {
      htitle = __cur.feedTitle ? __cur.feedTitle : getLang('audio_friends_feed');
    } else if (__cur.curSection && __cur.curSection == 'popular') {
      htitle = __cur.popularTitle ? __cur.popularTitle : getLang('audio_popular_audios');
    } else if (__cur.curSection && __cur.curSection.indexOf('owner') === 0) {
      htitle = __cur.allFriendsHTitles[__cur.audioFriend];
    } else if (from_search) {
      htitle = getLang('audio_title_search').replace('{q}', val(__cur.aSearch));
      pad_htitle = getLang('audio_title_search').replace('{q}', '<b>' + val(__cur.aSearch) + '</b>');
